...
gui.add(scene, 'environmentIntensity').min(1).max(10).step(0.5)
gui.add(scene, 'backgroundIntensity').min(1).max(10).step(0.5)
gui.add(scene, 'backgroundBlurriness').min(0).max(1).step(0.001)
gui.add(scene.backgroundRotation, 'y').min(0).max(Math.PI * 2).step(0.001).name('backgroundRotationY').onChange((step) => {
...
const tick = () =>
{
    ...
    if (holyDonut) {
        ...
        holyDonut.rotation.y = elapsedTime * 0.02
        holyDonut.rotation.z = elapsedTime * 0.02
